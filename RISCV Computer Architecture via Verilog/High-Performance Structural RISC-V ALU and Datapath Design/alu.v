...
module CLA4 (
    ...
    // Bit propagate / generate / xor-for-sum
    ...
    // Sum bits: S = (A xor B) xor carry_in_of_bit
    ...
    // Gblock = g3 + g2*p3 + g1*p2*p3 + g0*p1*p2*p3
    ...
    // Carry into MSB of this 4-bit block
...
module ALU (
    ...
    generate
        for (i = 0; i < 32; i = i + 1) begin : GEN_ALU_BITS
            ...
            GATE_XOR u_xor  (.a(A[i]), .b(B[i]), .y(xor_out[i]));
            GATE_XOR u_bsel (.a(B[i]), .b(ALUControl[2]), .y(b_arith[i]));
        end
    endgenerate

    // Shared arithmetic unit: 010 = ADD, 110 = SUB, 111 = SLT uses subtraction path
    CLA32 u_cla32 (
        .A(A),
        .B(b_arith),
        .Cin(ALUControl[2]),
    ...
    // SLT (signed): set = sum[31] xor overflow
    ...
    // Result selection
    // low01     : 000->AND, 001->XOR
    // low_group : 000/001/010 selected with ALUControl[1]
    // high_group: 110->SUM(SUB), 111->SLT selected with ALUControl[0]
    // final     : low_group vs high_group selected with ALUControl[2]
    generate
        for (i = 0; i < 32; i = i + 1) begin : GEN_RESULT_MUX
            GATE_MUX2 u_m0 (.d0(and_out[i]), .d1(xor_out[i]), .s(ALUControl[0]), .y(low01[i]));
            GATE_MUX2 u_m1 (.d0(low01[i]),   .d1(sum_out[i]), .s(ALUControl[1]), .y(low_group[i]));
            GATE_MUX2 u_m2 (.d0(sum_out[i]), .d1(slt_out[i]), .s(ALUControl[0]), .y(high_group[i]));
            GATE_MUX2 u_m3 (.d0(low_group[i]), .d1(high_group[i]), .s(ALUControl[2]), .y(Result[i]));
        end
    endgenerate

    // Overflow output is meaningful only for arithmetic operations (010/110/111)
    GATE_MUX2 u_ovf_out (.d0(GND), .d1(overflow_raw), .s(ALUControl[1]), .y(Overflow));
